...
import { getDashboardById, getUserDashboards } from "@/lib/business"
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-8">
      <SectionHero
        ...
        primaryAction={{ href: "/business/orders", label: "Manage Orders" }}
        secondaryAction={{ href: "/business/inventory", label: "View Inventory" }}
      >
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <aside className="rounded-[36px] border border-[var(--border)] bg-[linear-gradient(160deg,rgba(18,22,22,0.98),rgba(24,32,31,0.92))] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.24)]">
          ...
          <div className="mt-5 flex flex-col gap-3">
            <Link href="/business/inventory" className="rounded-[24px] bg-[var(--accent)] px-4 py-3 text-sm font-semibold text-[#08100c] transition hover:bg-[#dbffda]">
              Open inventory workspace
            </Link>
            <Link href="/business/orders" className="rounded-[24px] border border-[var(--border)] bg-white/4 px-4 py-3 text-sm font-medium text-[var(--foreground)] transition hover:bg-white/6">
              Check pending orders
            </Link>
            <Link href="/business/customers" className="rounded-[24px] border border-[var(--border)] bg-white/4 px-4 py-3 text-sm font-medium text-[var(--foreground)] transition hover:bg-white/6">
              Review customer activity
            </Link>
            <Link href="/business/reports" className="rounded-[24px] border border-[var(--border)] bg-white/4 px-4 py-3 text-sm font-medium text-[var(--foreground)] transition hover:bg-white/6">
